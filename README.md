...
### 3. It's all Promises.
itty `await`s every handler, looking for a return value.  If it gets one, it breaks the flow and return your value.  If it doesn't, it continues processing handlers/routes until we do.  This means that every handler can either be synchronous or async - it's all the same.
